...
window.addEventListener("load", function() {
    ...
    function l(e) {
        document.getElementById("recipe-name").textContent = e.name,
        document.getElementById("recipe-description").textContent = e.description;
        const d = document.getElementById("recipe-image");
        d && e.imgCover && (d.src = e.imgCover,
        d.alt = e.name),
        document.getElementById("rating-average").textContent = e.ratingsAverage,
        document.getElementById("rating-quantity").textContent = `(${e.ratingsQuantity} reviews)`,
        document.getElementById("prep-time-display").textContent = e.prepTime,
        document.getElementById("cook-time-display").textContent = e.cookTime,
        document.getElementById("servings-display").textContent = e.servings,
        document.getElementById("difficulty-badge").textContent = e.difficulty,
        document.getElementById("category-badge").textContent = e.category;
        const i = document.getElementById("ingredients-list");
        i.innerHTML = "",
        e.ingredients.forEach( (m, o) => {
            const r = document.createElement("li");
            r.className = "flex items-start",
            r.innerHTML = `
                <div class="w-6 h-6 bg-orange-500 rounded-full flex items-center justify-center text-white text-xs font-bold mt-0.5 mr-3 flex-shrink-0">${o + 1}</div>
                <span class="text-gray-700">${m}</span>
            `,
            i.appendChild(r)
        }
        );
        const p = document.getElementById("instructions-list");
        p.innerHTML = "",
        e.instructions.forEach( (m, o) => {
            const r = document.createElement("div");
            r.className = "flex items-start",
            r.innerHTML = `
                <div class="w-12 h-12 bg-orange-500 rounded-2xl flex items-center justify-center text-white text-xl font-bold mr-4 flex-shrink-0">${o + 1}</div>
        ...
        document.getElementById("calories-value").textContent = e.nutrition.calories,
        document.getElementById("protein-value").textContent = e.nutrition.protein,
        document.getElementById("carbs-value").textContent = e.nutrition.carbs,
        document.getElementById("fat-value").textContent = e.nutrition.fat,
        document.getElementById("fiber-value").textContent = e.nutrition.fiber,
        document.getElementById("sodium-value").textContent = e.nutrition.sodium;
        const u = document.getElementById("tips-list");
        u.innerHTML = "",
        e.tips.forEach(m => {
            const o = document.createElement("div");
            o.className = "flex items-start p-4 bg-amber-50 rounded-xl border-l-4 border-amber-400",
            o.innerHTML = `
        ...
        const h = document.getElementById("time-warning");
        e.totalTime > 45 ? h.classList.remove("hidden") : h.classList.add("hidden"),
        document.getElementById("recipe-display-section").classList.remove("hidden"),
        document.getElementById("recipe-display-section").scrollIntoView({
            behavior: "smooth"
        })
    }
    ...
    t.forEach(e => {
        e.addEventListener("click", function() {
            ...
            t.forEach(i => {
                i.classList.remove("text-orange-600", "border-orange-600"),
